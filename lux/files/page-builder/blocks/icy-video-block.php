<?php
/* Image Block */
if(!class_exists('ICY_Video_Block')) {
	class ICY_Video_Block extends AQ_Block {
		...
		function form($instance) {
			...
			<p class="description">
				<label for="<?php echo $this->get_field_id('video_url') ?>">
					Video Url(Enter Youtube Url only)<br/>
					<?php echo aq_field_input('video_url', $block_id, $video_url) ?>
				</label>
			</p>
            <p class="description">
		...
		function block($instance) {
			...
			$videoId =  explode("v=",$video_url); 
			$videoUrl = "http://www.youtube.com/embed/".$videoId[1];
			...
			<div id="bg-<?php echo $block_id; ?>" style="
                   ...
                   <iframe width="<?php echo $width; ?>" height="<?php echo $height; ?>" frameborder="0" allowfullscreen="true" src="<?php echo $videoUrl;?>"></iframe>
             </div>      
            <?php 
			}
			else
           ...
           <script type="text/javascript">
            ...
            <style type="text/css">
			  ...
			  #bg-<?php echo $block_id; ?> { <?php if($width){ ?>width:<?php echo $width; ?>px; <?php }?> <?php if($position){ ?>position:<?php echo $position;?>;<?php }?> <?php if($position_top){ ?>top: <?php echo $position_top; ?>px; <?php }?> <?php if($position_left){ ?>left: <?php echo $position_left; ?>px; <?php }?><?php if($border_color){ ?>border:<?php if($border_thickness) echo $border_thickness.'px'; ?><?php if($border_style) echo ' '.$border_style; ?><?php echo $border_color;?>;<?php }?> }
			</style>
          
             <div id="parallax-<?php echo $block_id; ?>" style="
				   <?php if($width){ ?>width:<?php echo $width; ?>px; <?php }?>
				   <?php if($height){ ?>height:<?php echo $height; ?>px; <?php }?>
				   top: <?php echo $position_top; ?>px;
				   left: <?php echo $position_left; ?>;
