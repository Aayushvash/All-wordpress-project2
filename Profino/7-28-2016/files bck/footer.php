    ...
    <div class="centering">
      <div class="mid">
        ...
        <div class="right">
          ...
          <?php echo do_shortcode('[contact-form-7 id="137" title="Jetzt kostenlos für den profino Newsletter anmelden!"]'); ?> 
...
<script src="<?php echo get_template_directory_uri(); ?>/js/appear.js" type="text/javascript"></script> 
...
$("document").ready(function($){
	...
	<?php } ?>	
		...
		/*if ($(window).width() <= 960 ) {
		$('.all-barp').parallax("50%", 0.1);
		$('.wird-barp').parallax("50%", 0.1);
		}
		else {		

		$('.wird-barp').parallax("50%", 0.2);
		$('.wird-barp').parallax("50%", 0.1);
		} */
...
$(window).on('load resize', function () {
   ...
   if ($(window).width() < 960 ) {
		...
		$('.header-bar').removeClass('header-barp');
		$('.all-barp').removeClass('all-barp');
		$('.wird-bar').removeClass('wird-barp');
		$('.ger-bar').removeClass('ger-barp');
		
	} else {
		
		$('.header-barp').parallax("50%", 0.6);
		$('.all-barp').parallax("50%", 0.2);
		$('.wird-barp').parallax("50%", 0.2);
		$('.ger-barp').parallax("50%", 0.1);
	}
});	
...
/* <![CDATA[ */
var google_conversion_id = 947503381;
...
<img height="1" width="1" style="border-style:none;" alt="" src="//googleads.g.doubleclick.net/pagead/viewthroughconversion/947503381/?value=0&amp;guid=ON&amp;script=0"/>
</div>
</noscript>    
