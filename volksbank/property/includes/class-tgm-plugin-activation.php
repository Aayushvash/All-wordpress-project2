...
'installing' => __( 'Installing Plugin: %s', 'bobox' ),
...
'complete' => __( 'All plugins installed and activated successfully. %1$s', 'bobox' ),
 ...
 public function notices() {
            ...
            if ( ! empty( $message ) ) {
                ...
                foreach ( $message as $type => $plugin_groups ) {
                    ...
                    $imploded = empty( $plugin_groups ) ? '<em>' . $last_plugin . '</em>' : '<em>' . ( implode( ', ', $plugin_groups ) . '</em> und <em>' . $last_plugin . '</em>' );

                    $rendered .= '<p>' . sprintf( __( $this->strings[$type], $count, 'tgmpa' ), $imploded, $count ) . '</p>';
                }

                $show_install_link = $install_link ? '<a href="' . add_query_arg( 'page', $this->menu, admin_url( 'themes.php' ) ) . '">' . $this->strings['install_link'] . '</a>' : '';
                $show_activate_link = $activate_link ? '<a href="' . add_query_arg( 'page', $this->menu, admin_url( 'themes.php' ) ) . '">' . $this->strings['activate_link'] . '</a>' : '';
...
class TGMPA_List_Table extends WP_List_Table {
 ...
 protected function _gather_plugin_data() {
            ...
            foreach ( TGM_Plugin_Activation::$instance->plugins as $plugin ) {
                ...
                $table_data[$i]['type'] = isset( $plugin['required'] ) && $plugin['required'] ? __( 'Required', 'bobox' ) : __( 'Recommended', 'bobox' );

                if ( ! isset( $installed_plugins[$plugin['file_path']] ) ) {
                    $table_data[$i]['status'] = sprintf( '%1$s', __( 'Not Installed', 'bobox' ) );
                } elseif ( is_plugin_inactive( $plugin['file_path'] ) ) {
                    $table_data[$i]['status'] = sprintf( '%1$s', __( 'Installed But Not Activated', 'bobox' ) );
                }
...
protected function _get_plugin_data_from_name( $name, $data = 'slug' ) {
        ...
        public function column_plugin( $item ) {
            ...
            if ( ! isset( $installed_plugins[$item['file_path']] ) ) {
                $actions = array(
                    'install' => sprintf(
                        '<a href="%1$s" title="' . __( 'Install', 'bobox' ) . ' %2$s">' . __( 'Install', 'bobox' ) . '</a>',
                        wp_nonce_url(
                            add_query_arg(
                                array(
       ...
       public function get_columns() {
        ...
        public function get_bulk_actions() {
            ...
            $actions = array(
                'tgmpa-bulk-install' => __( 'Install', 'bobox' ),
                'tgmpa-bulk-activate' => __( 'Activate', 'bobox' ),
            );
        ...
        public function process_bulk_actions() {
            ...
            if ( 'tgmpa-bulk-install' === $this->current_action() ) {
                ...
                if ( empty( $plugin_installs ) ) {
                    echo '<div id="message" class="error"><p>' . __( 'No plugins are available to be installed at this time.', 'bobox' ) . '</p></div>';
                    return false;
                }
