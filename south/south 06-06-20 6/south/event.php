...
$date = DateTime::createFromFormat($format_in, get_field('event_date',get_the_ID()));
		...
		<div class="info">
		  <div class="day"> 
		<h1><?php if ($date) {  echo $date->format($format_out); ?><?php } else {   ?><?php } ?><span><?php if ($date) {  echo $date->format($format_o); ?><?php } else {   ?><?php } ?></span></h1>
	   </div>
	   <h2> <?php the_title(); ?> <br/>
	   <small><?php if ($date) { echo $date->format($format_m);  ?> <?php } else {   ?><?php } ?></small> </h2>
	   </div>
