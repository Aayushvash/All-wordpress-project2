...
<script type="text/javascript">
  $(function() { 
	...
	?>
<object
	classid="clsid:D27CDB6E-AE6D-11cf-96B8-444553540000" width="50" height="50" id="FlashID" title="sdfsdf" style="display: none">
...
<div id="wrapper">
	...
	<div id="layout">
		...
		<div id="banner">
			...
			<div class="bannerBar">
				...
				<div class="head">
					...
					<?php echo $boat["Year"];?>
					<?php echo $boat["Make"];?>
						<span><?php echo $boat["Model"];?> </span> <span class="stockNoHead">Stock Number:&nbsp;&nbsp;<?php echo $boat["stockNo"];?> </span>
						<!-- Lockerz Share BEGIN -->
						<div class="a2a_kit a2a_default_style">
							<a class="a2a_dd" href="http://www.addtoany.com/share_save" style="font-size: 10px">Share</a> <span class="a2a_divider"></span> <a
								class="a2a_button_facebook"></a> <a class="a2a_button_twitter"></a> <a class="a2a_button_email"></a> <a class="a2a_button_digg"></a> <a
								class="a2a_button_delicious"></a>
						</div>
					...
					<h2>
					<?php echo '1-877-889-2628';//echo $boat["BrokerTel"];?>
						<a href="#LiOne">Contact</a>
					</h2>
						...
						<div class="ad-nav">
							<div class="ad-thumbs">
								...
								while($image = mysql_fetch_array($imagedata)) {?>
									<li><a href="<?php echo $image['ImageURL']; ?>"><img src="<?php echo $image['ImageURL']; ?>" alt="" width="80" height="50"
											class="image<?php echo $coutImgtag;?>" /> </a></li>
											<?php
											$coutImgtag++;
								}?>
								</ul>
			...
			<div id="centerSide">
				...
				<?php echo $boat['Description'] ?>
					...
					<?php if($video['VideoURL'] && $video['VideoURL']!=''){?>
						<a href="#"> <object width="100%" height="100%" codebase="http://download.macromedia.com/pub/shockwave/cabs/flash/swflash.cab#version=6,0,29,0"
								...
								<embed width="585" height="380" wmode="transparent" type="application/x-shockwave-flash" pluginspage="http://www.macromedia.com/go/getflashplayer"
									quality="high" src="http://www.vidbrochure.com/commercial.swf?commercialid=<?php echo $video['VideoURL'];?>&amp;domainid=vidbrochure">
		...
		<div id="footer">
			...
			<div class="footerBar">
				...
				<div class="center">
					...
					<ul>
						...
						<li>Holding: <?php if ($boat['HoldingTankNo'] != "0"){ echo $boat['HoldingTankNo'] . " x " . $boat['HoldingTankCap'] . " " . $boat['HoldingTankCapUnit']; } ?>
						</li>
					</ul>
				</div>
			</div>
			...
			while($feature = mysql_fetch_array($featuredata)) {
				$feature['FeatureData']=str_replace("â","'",$feature['FeatureData']);
	...
	<!--  \ layout / -->
			...
			else
			{
				...
				if (@$_POST['stockNo'] && $_POST['stockNo'] != ""){
	...
	<div id="layout">
		...
		<div id="content">
			...
			<div id="centerSide">
				...
				<form name="frmNoPage" id="frmNoPage" class="fromlisting" action="<?php echo $searchurl;?>" method="post">
					...
					<select name="currency" style="width: 55px">
						...
						<div class="option">
							...
							<div class="selectopt">
								...
								<?php if (!$_POST['location'] || $_POST['location'] == ""){?>
									...
									/*$Querysearch = "SELECT DISTINCT LocationCity, LocationState FROM `".$table_prefix."bv_boatdetails` ORDER BY LocationCity";
						...
						<?php
						if(isset($_POST['featuredSearch'])&& $_POST['featuredSearch']==1)
						{
							echo '<input type="hidden"  name="featuredSearch" value="1"/>';
						}
						?>
						<div class="option">
...
<form name="frmNoPage" action="" method="post">-->
					<div class="clear"></div>
					<input type='hidden' id='current_boat_page' /> <input type='hidden' id='show_per_page' /> <input type='hidden' id='number_of_page' /> <input type="hidden"
						name="page" id="noPage" value="<?php echo $noPage;?>" />
					<!--</form>             
			  <form name="sortandfilterSort" class="" action="" method="post">-->
					<div id="sort">
						Sort By: <select name="sort" onchange="sortBy();">
				...
				<div class="galleryBar">
					...
					while($info = mysql_fetch_array($data_p)) { ?>
						<li>
							...
							while($image = mysql_fetch_array($imagedata)) {?>
								<a href="<?php echo $mainPath.HtAccessURL('inventory',$info['Make'].':'.$info['Model'].':'.$info['BoatID']);?>" /> <img
									src="<?php echo $timpath; ?>?src=<?php echo $image['ImageURL'];?>&w=330&h=245&zc=1&q=100" alt="<?php echo $info['Make'] .'&nbsp;'. $info['Model'];?>" />
								</a>
								<?php }?>
							</div>
							...
							<div class="center">
								...
								<h2>
									...
									</a><br /> <span class="stockListing"><label>Stock Number:</label> <?php echo $info['stockNo'];?> </span>
									<div class="clear"></div>
								...
								$imagedata = mysql_query($Queryboat);
								while($image = mysql_fetch_array($imagedata)) {?>
									<li defaultSrc="<?php echo $timpath; ?>?src=<?php echo $image['ImageURL'];?>&w=330&h=245&zc=1&q=100"><img
										src="<?php echo $timpath; ?>?src=<?php echo $image['ImageURL'];?>&w=85&h=70&zc=1&q=100" alt="<?php echo $info['Make'] .'&nbsp;'. $info['Model'];?>" />
									</li>
									<?php }?>
								</ul>
							...
							<div class="right">
								...
								<ul>
									<li class="photo"><a href="<?php echo $mainPath.HtAccessURL('inventory',$info['Make'].':'.$info['Model'].':'.$info['BoatID']);?>"><?php echo $totaleImg;?>
											Photos</a></li>
									<li class="view"><a href="<?php echo $mainPath.HtAccessURL('inventory',$info['Make'].':'.$info['Model'].':'.$info['BoatID']);?>" />View Detail</a>
									</li>
