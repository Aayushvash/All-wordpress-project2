		...
		//	console.log("Found product"  + rel);
			...
			jQuery(this).parent().parent().parent().find(".my" + rel).each(function(){
				...
				if(jQuery(this).data("rel")== rel && jQuery(this).val()!= "") {
					...
					totalitem += jQuery.trim(jQuery(this).val()) * 1;
					//console.log("eeee :: " + totalitem);
				}
			});
			var totalcount =dishall* qty;
			if(totalcount < totalitem) {
				alert("You need to select " + (totalitem - (totalcount)  ) + " more options for Tranditional Party Packs");
				returnvalue= false;
			}else if (totalcount > totalitem) {
				alert("Please select only " + (totalcount) +  " option for your Tranditional Party Packs");
				returnvalue= false;
			}
		}
	});
...
function orderConfirmation()
{
	...
	if(first=='' || last=='' || address=='' || city=='' || state=='' || zip=='' || phone=='' || email=='' || email2=='' || date=='' || timer=='' || notes=='')
	{
...
if ($_SERVER['REQUEST_METHOD']=='POST') {	
	if(isset($_POST['carttotal']) && is_numeric($_POST['carttotal'])) {
		...
		$to .= $email;
		$subject = 'Mr. Chicken Order : '. date("m-d-Y H:i:s");
		// message
		$message = '
		<html>
		<head>
		...
		<tr><td>Email2:</td><td>'.$_POST['email2'].'</td></tr>
		<tr><td>Date:</td><td>'.$_POST['date'].'</td></tr>
		<tr><td>Timer:</td><td>'.$_POST['timer'].'</td></tr>
				...
				foreach ($cartview as $k=>$v) {
					...
					if(is_array($v)) {
						foreach($v as $k1=>$v1) {
							...
							$qty = 0;
							if(isset($_POST['item'][$k1]['quantity']) && is_numeric($_POST['item'][$k1]['quantity'])) {
								$message .= '<td>'.$_POST['item'][$k1]['quantity'] .'</td>';
								$qty=$_POST['item'][$k1]['quantity'];
							}else {
  ...
  &nbsp;<table class="cartview">
		<tbody>
			...
			<?php 
				$total = 0;
				foreach ($cartview as $k=>$v) {
					echo "<tr><td colspan=\"4\"><h2>". $k .'</h2></td></tr>';
					if(is_array($v)) {
						foreach($v as $k1=>$v1) {
							...
							if(count($v1) > 4) { //Combined product only
								echo '<td>'.$v1['quantity'] .'
								<input type="hidden" id="quantity0'. $k1. '" name="item['. $k1. '][quantity]" value="'.$v1['quantity'] .'" class="cartcalculateprice" data-price="'.$v1['price'].'" /></td>';
							} else {
								echo '<td><input type="text" id="quantity0'. $k1. '" name="item['. $k1. '][quantity]" value="'.$v1['quantity'] .'" class="cartcalculateprice" data-price="'.$v1['price'].'" size="4"/></td>';
							}
			...
			?>
			<tr><td colspan="3" align="right"><h3>Total Amount : &nbsp;</h3>  </td> 
				<td><h3 class="carttotal"> $ <?php echo number_format($total,2); ?></h3></td></tr> 
	...
	<tr>
    <td></td>
    <td><a class="confirmorder" href="#confirm">Confirm Order</a></td>
    <td><a href="<?php echo get_permalink($thispageid); ?>">Change Order</a></td>
	<td></td>
  </tr>
 </tbody> 
</table>  
...
<table style="display:none;" class="orderform">
    ...
    <p style="display:none;" class="text orderform"><span style="font-weight:bold;">When would you like your order ready by?</span><i> Note: Orders less than 24 hours please call our number to confirm we can have it prepared in time. </i><br>216-409-3517</p>
    
    <table style="display:none;" class="orderform">
    <tbody><tr>
    <td class="formrt">Date:</td>
    <td><input type="text" id="date" value="" class="datepicker hasDatepicker" name="date"></td>
    </tr>
    <tr>
    ...
    <tr>
    <td colspan="2">	
		<input type="submit" value="Confirm Order" name="frmConfirmOrder">	<br>	<div style="text-align:center"><a class="backtocartview" href="#cartview">View Order</a></div>    
    </td>
    </tr>
    </tbody></table>
    <br>
	...
	<?php for($i=1; $i<=2; $i++){  ?>
        ...
        <article class="sandwich-block  <?php echo (($i==2)?'ordersand':''); ?>">
            ...
            <div class="post" id="post-<?php the_ID(); ?>">            
				...
				<table width="100%">
				<tr>
					<td class="carttotalshortview">
					</td>
					<td>
					<input type="submit" class="submitbtn" name="btnSubmit" value="Confirm Order"  onclick="return bis()" />
					</td>
				</tr>
				</table>
